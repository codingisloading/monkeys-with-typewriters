...
## 📋 What is monkeys-with-typewriters?
...
- If there is an `.exe` file, download it. This is the easiest way to install.
- If it is a `.zip` file, download it and extract the contents. To extract, right-click the file and choose **Extract All**. Follow the prompts to unzip the folder.

### 4. Run the Software

- If you have an `.exe` file, double-click it to start the installation.
- Follow the on-screen instructions to complete installation. It should only take a few minutes.
...
1. Connect the microcontroller to your PC using a USB cable.
2. In the monkeys-with-typewriters program, open the **Device Setup** menu.
3. Select your microcontroller model from the list.
4. Click **Connect** to link your PC and the device.
5. Upload the AI model to the microcontroller following the program’s instructions.

Your device will now run AI tasks reliably and with very low delay.

---

## 🧰 How to Use monkeys-with-typewriters
...
## 🔄 Updating monkeys-with-typewriters
